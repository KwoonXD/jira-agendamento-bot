...
# ↪ import do componente FullCalendar
...
with tab_cal:
    st.header("📆 Calendário Arrastável de Agendamentos")

    # Monta lista de eventos pro FullCalendar
    events = []
    for issue in agendados_raw:
        ...
        events.append({
            ...
            "end":     end.isoformat(),
            "extendedProps": {"loja": loja}
        })

    # renderiza o calendário, habilita drag&drop e evento de recalendarização
    calendar_response = FullCalendar(
        events=events,
        editable=True,           # habilita arrastar
        droppable=False,         # não vamos droppar de fora
        initial_view="dayGridMonth",
        height="700px"
    )

    # quando o usuário soltar um evento em outra data, o componente retorna um JSON
    if isinstance(calendar_response, dict) and calendar_response.get("action") == "eventDrop":
        ev = calendar_response["event"]
        key = ev["id"]
        new_start = ev["start"]           # ex: "2025-06-25T15:00:00-03:00"
        # converte para o formato Jira
        dt_jira = datetime.fromisoformat(new_start).strftime("%Y-%m-%dT%H:%M:%S.000-0300")
        # envia update direto no Jira
        res = jira.transicionar_status(
            key,
            None,           # sem transição de status, apenas update de campo
            fields={"customfield_12036": dt_jira}
        )
