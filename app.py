...
with st.sidebar:
    ...
    if loja_sel != "—":
        ...
        if em_campo:
            ...
            sel = st.multiselect("Selecione FSAs pend.+agend.:", sorted(set(opts)))
...
# ── Main com Tabs: Lista vs Calendário ──
...
with tab_cal:
    st.header("📅 Visão de Calendário de Agendamentos")
    rows = []
    for issue in agendados_raw:
    ...
    if not rows:
        ...
        fig = px.timeline(
            ...
            color="Loja",
            hover_name="FSA",
            title="Agenda de Chamados por Loja"
        )
