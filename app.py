...
if not st.session_state.authenticated:
    ...
    if submitted:
        u = user.strip()
        p = pwd.strip()
        # DEBUG: mostra o que veio (remova após confirmar)
    ...
    # Fora do form, mas ainda antes de st.stop():
...
with st.sidebar:
    ...
    pend = jira.buscar_chamados("project = FSA AND status = AGENDAMENTO", FIELDS_SIMPLE)
    age  = jira.buscar_chamados('project = FSA AND status = AGENDADO', FIELDS_SIMPLE)
