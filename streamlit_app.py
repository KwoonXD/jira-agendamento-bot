...
def parse_dt(raw):
    if not raw:
        return "Não definida"
    try:
        return datetime.strptime(raw, "%Y-%m-%dT%H:%M:%S.%f%z").strftime("%d/%m/%Y")
    except Exception:
        # fallback sem microssegundos/ timezone
        try:
            return datetime.strptime(raw, "%Y-%m-%dT%H:%M:%S%z").strftime("%d/%m/%Y")
        except Exception:
            return "Não definida"
...
with st.sidebar:
    ...
    # Conjunto seguro de lojas vindas de todos os grupos
    ...
    if loja_sel != "—":
        ...
        if em_campo:
            ...
            # chaves por loja em todos os grupos
            ...
            if st.button(f"Agendar e mover {len(all_keys)} FSAs"):
                ...
                # b) mover todos conforme técnico
                for k in all_keys:
                    if tem_tecnico:
                        trans = jira.get_transitions(k)
