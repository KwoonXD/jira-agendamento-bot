...
class JiraAPI:
    ...
    def buscar_chamados(self, jql: str, fields: str) -> list:
        ...
        res = requests.get(url, headers=self.headers, auth=self.auth, params=params, timeout=25)
    ...
    def get_transitions(self, issue_key: str) -> list:
        ...
        res = requests.get(url, headers=self.headers, auth=self.auth, timeout=25)
        if res.status_code == 200:
    ...
    def get_issue(self, issue_key: str) -> dict:
        ...
        res = requests.get(url, headers=self.headers, auth=self.auth, params={"fields": "status"}, timeout=25)
        if res.status_code == 200:
            return res.json()
        return {}
    ...
    def transicionar_status(self, issue_key: str, transition_id: str, fields: dict = None) -> requests.Response:
        ...
        url = f"{self.jira_url}/rest/api/3/issue/{issue_key}/transitions"
        res = requests.post(url, headers=self.headers, auth=self.auth, json=payload, timeout=25)
        return res
